...
<div class="cart_totals uspsdiv">
    ...
    <!--<table class="table table-striped fedexdiv " id="shiptype1">
        ...
        <tr>
            ...
            <td class="text-right">
                <select style="width:320px; height: 30px;" name="states">
        ...
        <tr>
            ...
            <td class="text-right">
                <select style="width:320px; height: 30px;" name="ServiceType">
    ...
    $("#submitShipups").click(function () {
        ...
        $.ajax({
            ...
            success: function (msg) {
                ...
                if (msg.status == 1) {
                    $("#adshare-checkout-shipping").html(msg.price);
                    var price = $('input[name=subtotal]').val();
                    var price_with_shipment = (parseFloat(price)+parseFloat(msg.price)).toFixed(2);
                    $('input[name="adshare[adshare_price]"]').val(price_with_shipment);
                    // Set total price with shipment (shipment)
                    $(".adshare-checkout-shipping").html(msg.price);
                    $(".adshare-checkout-total").html(price_with_shipment);

                    // Set shipment status
                    if($('input[name=ship_status]').val()==''){
                        $('input[name=ship_status]').val('1');
                    }

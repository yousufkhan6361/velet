...
class Packages2 extends MY_Controller {
    ...
    public function payment(){
        ...
        //debug($data['packages']['packages_id'],1);

        $data['packageid'] = $data['packages']['packages_id'];
 ...
 //       debug($orderData);
    ...
    public function package_payment(){
        ...
        {
            // if (($stripe instanceof MyClass) != true ) {
            ...
            $par449['where']['signup_id'] = $this->userid;
            $userdata = $this->model_signup->find_one_active($par449);

            $par4556['where']['order_id'] = $oid;
            $orderDetail = $this->model_order->find_one($par4556);
            ...
            $token = $_POST['stripeToken'];
                    ...
                    try{
                        $charge = $stripe->charges->create([
                            'amount' => $package['packages_price']*100,
                            'currency' => 'gbp',
                    ...
                    if($charge->id != NULL)
                    {
                            ...
                            $param['order_response'] = serialize($charge);
                            $param['order_paymentID'] = $subscription->id;
                            $param['order_payment_status'] = 1;
                            $this->model_order->update_by_pk($oid,$param);

                            if (ENVIRONMENT != "development") 
                            {
                                $template = $this->load->view('_layout/email_template/invoice3',$data,true);
                                $title = 'Membership Confirmation - Invoice #' . $orderDetail['order_id'];
                                $to = $userdata['signup_email'];
                                parent::client_email_order($to,$template,$title);
                               
                            }

                            $json_param['status'] = 1;
                            $json_param['txt'] = 'Package Purchased';
